...
class NANDFlasher(TeensySerial):
	...
	def dump(self, filename, block_offset, nblocks):
		...
		for page in range(block_offset*self.NAND_PAGES_PER_BLOCK, (block_offset+nblocks)*self.NAND_PAGES_PER_BLOCK, 1):
			...
			fo.write(data)
			print("\r%d KB / %d KB"%((page-(block_offset*self.NAND_PAGES_PER_BLOCK)+1)*self.NAND_PAGE_SZ_PLUS_RAS//1024, nblocks*self.NAND_BLOCK_SZ_PLUS_RAS//1024), end="", flush=True)
	...
	def program(self, data, verify, block_offset, nblocks):
		...
		while block < nblocks:
			...
			print("\r%d KB / %d KB"%(((block+1)*self.NAND_BLOCK_SZ_PLUS_RAS)//1024, (nblocks*self.NAND_BLOCK_SZ_PLUS_RAS)//1024), end="", flush=True)
...
if __name__ == "__main__":
	...
	if (len(sys.argv) == 3) and (sys.argv[1] == "ps3badblocks"):
		...
		while block < nblocks:
			...
			print("\r%d KB / %d KB"%(((block+1)*(block_plus_ras_sz))//1024, (nblocks*(block_plus_ras_sz))//1024), end="", flush=True)
