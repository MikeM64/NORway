...
class NORFlasher(TeensySerial):
	...
	def verify(self, addr, data):
		...
		for offset in range(0, len(data)//2, BLOCK):
			...
			print("\r%d KB / %d KB"%((offset+BLOCK)//512, len(data)//1024), end="", flush=True)
...
def main():
	print("NORway.py v0.8 - Teensy++ 2.0 NOR flasher for PS3 (judges@eEcho.com)")
	...
	if len(sys.argv) == 1:
		...
		print()
		print("Examples:")
		print("  %s COM1".format(sys.argv[0]))
	...
	if len(sys.argv) == 4 and sys.argv[2] == "dump":
		...
		for offset in range(0, 0x800000, BLOCK):
			...
			print("\r%d KB / 16384 KB"%((offset+BLOCK)//512), end="", flush=True)
		print()
